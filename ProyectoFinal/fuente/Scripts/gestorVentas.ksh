...
while true; do
	...
	3)
		...
		easyTput colortexto rojo
		venta="$(bd -t "ventas")"
		ventasproductos="$(bd -t "ventasproductos")"
		print "$venta" | while read line; do
			easyTput colortexto rojo
			print "$line" | sed -e 's/:/\t/g'
			idActual="$(print "$line" | awk -F: '{print $1}')"
			val="$(print "$ventasproductos" | awk -F: -v idActual="$idActual" '($2 == idActual){printf "\t%s\n", $0}')"
			easyTput colortexto verde
			print "$val"
		done
